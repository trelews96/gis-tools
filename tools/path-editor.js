...
(function() {
    try {
        ...
        // SOI-enforced stage → allowed status combinations.
        // If workflow_status is being submitted and the feature's effective stage
        // matches a key here, the status must be in the corresponding Set.
        const SOI_STAGE_RULES = {
            'OSP_CONST': new Set(['NA','INPROG','ASSG','DNB','ONHOLD','CMPLT','QCINPROG','CLNTRJCT','QCCMPLT','DEFRD']),
            'ASBUILT':   new Set(['RDYFDLY','DLYCMPLT','INVCMPLT','CLNTDEL','CLNTRJCT','CLNTAPPV']),
        };
        ...
        async function submitFeature(){
            ...
            try{
                ...
                // SOI stage/status validation — use the form's stage if being edited,
                // otherwise fall back to the feature's current stored stage value.
                const effectiveStage = vals['workflow_stage'] ?? item.feature.attributes['workflow_stage'];
                const incomingStatus = vals['workflow_status'];
                if(incomingStatus !== undefined && effectiveStage && SOI_STAGE_RULES[effectiveStage]){
                    if(!SOI_STAGE_RULES[effectiveStage].has(incomingStatus)){
                        const allowed = [...SOI_STAGE_RULES[effectiveStage]].join(', ');
                        updateStatus('Blocked — invalid workflow status for current stage');
                        alert(`Cannot submit — Workflow Status "${incomingStatus}" is not allowed while Stage is "${effectiveStage}".\n\nAllowed statuses for ${effectiveStage}:\n  ${allowed}`);
                        return;
                    }
                }
        ...
        async function applyBulkEdit(){
            ...
            // SOI stage/status validation across all features in the bulk set
            const bulkIncomingStatus=vals['workflow_status'];
            if(bulkIncomingStatus!==undefined){
                const blocked=cfg.features.filter(f=>{
                    const effectiveStage=vals['workflow_stage']??f.attributes['workflow_stage'];
                    return effectiveStage&&SOI_STAGE_RULES[effectiveStage]&&!SOI_STAGE_RULES[effectiveStage].has(bulkIncomingStatus);
                });
                if(blocked.length){
                    const badStages=[...new Set(blocked.map(f=>vals['workflow_stage']??f.attributes['workflow_stage']))].join(', ');
                    updateStatus('Blocked — invalid workflow status for one or more features');
                    alert(`Cannot apply — Workflow Status "${bulkIncomingStatus}" is not valid for ${blocked.length} of ${cfg.features.length} feature(s).\n\nAffected stage(s): ${badStages}\n\nChoose a status that is valid for all features, or edit those features individually.`);
                    return;
                }
            }
