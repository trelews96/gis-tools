...
(function() {
    try {
        ...
        // Anchor midpoint state (right-click to plant)
        let arcAnchorMode = false;          // true after user right-clicks to set anchor
        let arcAnchorPt = null;             // the snapped point on the line
        let arcAnchorSegInfo = null;        // { pathIdx, segIdx, t, point } from findClosestPointOnPath
        let arcAnchorGraphic = null;        // diamond visual marker
        let arcActiveSide = 'left';         // 'left' | 'right' — which side the mouse is on
        let arcRightClickHandler = null;    // esri pointer-down handle for right-click
        let arcContextMenuHandler = null;   // DOM contextmenu suppressor
        ...
        /**
         * Find the closest point on any segment of geom to mp.
         * Returns { pathIdx, segIdx, t, point: {x,y} } where t ∈ [0,1] along that segment.
         */
        function findClosestPointOnPath(geom, mp) {
            let minD = Infinity;
            let result = { pathIdx: 0, segIdx: 0, t: 0, point: { x: mp.x, y: mp.y } };
            for (let pi = 0; pi < geom.paths.length; pi++) {
                const path = geom.paths[pi];
                for (let si = 0; si < path.length - 1; si++) {
                    const ax = path[si][0], ay = path[si][1];
                    const bx = path[si+1][0], by = path[si+1][1];
                    const C = bx-ax, D = by-ay, A = mp.x-ax, B = mp.y-ay;
                    const lenSq = C*C + D*D;
                    const t = lenSq ? Math.max(0, Math.min(1, (A*C+B*D)/lenSq)) : 0;
                    const px = ax+t*C, py = ay+t*D;
                    const d = Math.sqrt((mp.x-px)**2 + (mp.y-py)**2);
                    if (d < minD) { minD = d; result = { pathIdx: pi, segIdx: si, t, point: { x: px, y: py } }; }
                }
            }
            return result;
        }
        ...
        /**
         * Cumulative Euclidean path length from path start up to a fractional
         * position defined by (segIdx, t). Used to determine which side of
         * the anchor point the mouse cursor is on.
         */
        function pathLengthToPoint(geom, pathIdx, segIdx, t) {
            ...
            if (segIdx < path.length - 1) {
                const dx = path[segIdx+1][0]-path[segIdx][0], dy = path[segIdx+1][1]-path[segIdx][1];
                len += t * Math.sqrt(dx*dx + dy*dy);
            }
            return len;
        }

        /**
         * Returns 'left' if the mouse is on the start-side of the anchor,
         * 'right' if on the end-side, by comparing cumulative path lengths.
         */
        function getArcSide(geom, anchorInfo, mousePoint) {
            const anchorLen = pathLengthToPoint(geom, anchorInfo.pathIdx, anchorInfo.segIdx, anchorInfo.t);
            const mInfo     = findClosestPointOnPath(geom, mousePoint);
            const mouseLen  = pathLengthToPoint(geom, mInfo.pathIdx, mInfo.segIdx, mInfo.t);
            return mouseLen <= anchorLen ? 'left' : 'right';
        }

        /**
         * Build the full replacement path array for an anchor-mode arc apply.
         *
         * side === 'left':  arcPts covers start→anchor; original vertices from
         *                   after the anchor segment fill the right side.
         * side === 'right': original vertices from the start fill the left side
         *                   up to the anchor; arcPts covers anchor→end.
         *
         * Uses the line-specific anchorInfo so coincident lines with slightly
         * different vertex counts are handled correctly.
         */
        function buildAnchoredArcPath(geom, anchorInfo, arcPts, side) {
            const path = geom.paths[anchorInfo.pathIdx];
            const anchorCoord = [anchorInfo.point.x, anchorInfo.point.y];

            if (side === 'left') {
                // arcPts: start → anchor.  Tail: anchor segment end → path end.
                const tail = path.slice(anchorInfo.segIdx + 1).map(v => v.slice());
                return [...arcPts, ...tail];
            } else {
                // Head: path start → anchor segment start (inclusive).
                // arcPts: anchor → end.  Skip arcPts[0] if it duplicates anchorCoord.
                const head = path.slice(0, anchorInfo.segIdx + 1).map(v => v.slice());
                return [...head, anchorCoord, ...arcPts.slice(1)];
            }
        }

        // ── Arc anchor graphics ───────────────────────────────────────────────

        function showArcAnchorGraphic(pt) {
            clearArcAnchorGraphic();
            mapView.graphics.add({
                geometry: { type: 'point', x: pt.x, y: pt.y, spatialReference: pt.spatialReference || mapView.spatialReference },
                symbol: { type: 'simple-marker', style: 'diamond', color: [167,139,250,1], size: 13, outline: { color: [255,255,255,1], width: 2 } }
            });
            arcAnchorGraphic = mapView.graphics.getItemAt(mapView.graphics.length - 1);
        }
        function clearArcAnchorGraphic() {
            if (arcAnchorGraphic) { mapView.graphics.remove(arcAnchorGraphic); arcAnchorGraphic = null; }
        }
        ...
        function syncLockedFeature(newGeom) {
            ...
            try {
                if (lockedFeature.feature?.clone) { const c = lockedFeature.feature.clone(); c.geometry = newGeom; lockedFeature.feature = c; }
                else if (lockedFeature.feature) lockedFeature.feature.geometry = newGeom;
        ...
        function findAllCutInfos(lineGeom, snapPt, tolerance) {
            ...
            for (let pi = 0; pi < lineGeom.paths.length; pi++) {
                ...
                for (let si = 0; si < path.length - 1; si++) {
                    const a = { x: path[si][0], y: path[si][1] }, b = { x: path[si+1][0], y: path[si+1][1] };
        ...
        function splitLineMulti(lineGeom, sortedCutInfos, snapPt) {
            ...
            for (const ci of sortedCutInfos) { if (!cutsByPath.has(ci.pathIdx)) cutsByPath.set(ci.pathIdx, []); cutsByPath.get(ci.pathIdx).push(ci.segIdx); }
            const outputSegments = []; let currentPaths = [], currentVerts = [];
            for (let pi = 0; pi < lineGeom.paths.length; pi++) {
                const path = lineGeom.paths[pi], cutsHere = (cutsByPath.get(pi) || []).slice().sort((a,b)=>a-b); let vi = 0;
            ...
            if(currentVerts.length>=2)currentPaths.push(currentVerts.slice()); if(currentPaths.length)outputSegments.push(currentPaths.slice());
            return outputSegments.filter(paths=>paths.length>0&&paths.every(p=>p.length>=2)).map(paths=>buildPolyline(lineGeom,paths)).filter(g=>geodeticLength(g)>=MIN_SEGMENT_LEN_FT);
        }
        ...
        function handleHotkey(e) {
            ...
            switch(e.key){
                ...
                case 'Escape':
                    ...
                    // Arc: ESC clears just the anchor if set, then clears line selection, then exits mode
        ...
        async function findNearbyLinesForCut(pointGeom) {
            const buf=CUT_TOLERANCE_M,{x,y}=pointGeom,bufGeom={type:'polygon',spatialReference:pointGeom.spatialReference,rings:[[[x-buf,y-buf],[x+buf,y-buf],[x+buf,y+buf],[x-buf,y+buf],[x-buf,y-buf]]]},found=[];
            await Promise.all(lineLayers.filter(cfg=>cfg.layer.visible).map(async cfg=>{try{const res=await cfg.layer.queryFeatures({geometry:bufGeom,spatialRelationship:'intersects',returnGeometry:true,outFields:['*'],maxRecordCount:100});for(const f of res.features){const cutInfos=findAllCutInfos(f.geometry,{x,y},buf);if(cutInfos.length>0)found.push({feature:f,layer:cfg.layer,layerConfig:cfg,cutInfos});}}catch(e){console.error(`findNearbyLinesForCut error on ${cfg.name}:`,e);}}));
            return found;
        }
        ...
        async function handleCutClick(event){
            ...
            cutSelectedPoint=ptResult.feature;cutSelectedPointLayer=ptResult.layer;await highlightCutGeometry(cutSelectedPoint.geometry,true);
        ...
        async function findAllFeaturesAtClick(sp, mp) {
            const candidates=[], seenOids=new Set(), allCfgs=[...pointLayers,...lineLayers].sort((a,b)=>a.name.localeCompare(b.name)), hitLayerIds=new Set();
            ...
            if(uncoveredCfgs.length>0){const tol=POINT_SNAP_TOLERANCE*(mapView.resolution||1),ext=makeExt(mp.x,mp.y,tol,mapView.spatialReference);await Promise.all(uncoveredCfgs.map(async cfg=>{try{const res=await cfg.layer.queryFeatures({geometry:ext,spatialRelationship:'intersects',returnGeometry:true,outFields:['*'],maxRecordCount:20});for(const f of res.features){if(!f.geometry)continue;const oid=getOid(f);if(oid!=null&&seenOids.has(oid))continue;if(oid!=null)seenOids.add(oid);candidates.push({feature:f,layer:cfg.layer,layerConfig:cfg});}}catch(e){console.error(`findAllFeaturesAtClick fallback error on ${cfg.name}:`,e);}}))}
            return candidates;
        }
        ...
        async function handleDeleteClick(event) {
            if(deleteProcessing)return;hideDelContextMenu();updateStatus('🗑️ Finding features at click location…');
            const sp={x:event.x,y:event.y},mp=mapView.toMap(sp);deleteCandidates=await findAllFeaturesAtClick(sp,mp);
            if(!deleteCandidates.length){updateStatus('❌ No features found at this location.');return;}
            const eligible=deleteCandidates.filter(c=>getDeleteFieldName(c.layer)!==null),ineligible=deleteCandidates.length-eligible.length;
            if(!eligible.length){updateStatus(`❌ None of the ${deleteCandidates.length} feature(s) found have a "${DELETE_FIELD}" field.`);return;}
            deleteCandidates=eligible;deleteSelectedIndices=new Set(deleteCandidates.map((_,i)=>i));
            const listEl=delCtxMenu.querySelector('#delCtxList');listEl.innerHTML='';const typeIcon=t=>t==='point'?'📍':t==='polyline'?'〰️':'⬡';
            for(let i=0;i<deleteCandidates.length;i++){const c=deleteCandidates[i],oid=getOid(c.feature)??'?',gtype=c.feature.geometry?.type??'unknown',fieldName=getDeleteFieldName(c.layer),curVal=c.feature.attributes?.[fieldName]??'null',alreadyDeleted=String(curVal).toUpperCase()==='YES',row=document.createElement('label');row.style.cssText='display:flex;align-items:flex-start;gap:6px;padding:6px 10px;cursor:pointer;border-bottom:1px solid #1e1935;user-select:none;';const cb=document.createElement('input');cb.type='checkbox';cb.checked=true;cb.dataset.idx=String(i);cb.style.cssText='margin-top:2px;cursor:pointer;flex-shrink:0;';const info=document.createElement('div');info.style.cssText='flex:1;font-size:11px;line-height:1.5;';info.innerHTML=`<strong style="color:${alreadyDeleted?'#fca5a5':'#e2d9f3'};">${typeIcon(gtype)} ${c.layerConfig.name}</strong><div style="color:#7a6d96;font-size:10px;">OID: ${oid} · ${DELETE_FIELD}: ${curVal}${alreadyDeleted?' ⚠️ already flagged':''}</div>`;const dot=document.createElement('span');dot.textContent='●';dot.style.cssText='color:#ef4444;font-size:14px;flex-shrink:0;margin-top:1px;';cb.addEventListener('change',()=>{const idx=parseInt(cb.dataset.idx);if(cb.checked){deleteSelectedIndices.add(idx);dot.style.color='#ef4444';}else{deleteSelectedIndices.delete(idx);dot.style.color='#3d3268';}updateDelExecuteBtn();});row.addEventListener('mouseenter',()=>{row.style.background='#2d1b0a';showPickerHoverHighlight(c.feature.geometry);});row.addEventListener('mouseleave',()=>{row.style.background='';clearPickerHoverHighlight();});row.appendChild(cb);row.appendChild(info);row.appendChild(dot);listEl.appendChild(row);}
            delCtxMenu.querySelector('#delCtxCount').textContent=deleteCandidates.length;const selAllBtn=delCtxMenu.querySelector('#delCtxSelectAll');if(selAllBtn){selAllBtn.textContent='✓ All';selAllBtn.onclick=()=>{const rows=[...listEl.querySelectorAll('label')],allOn=deleteSelectedIndices.size===deleteCandidates.length;rows.forEach((_,i)=>{const cb=rows[i].querySelector('input'),dot=rows[i].querySelector('span');cb.checked=!allOn;if(!allOn){deleteSelectedIndices.add(i);if(dot)dot.style.color='#ef4444';}else{deleteSelectedIndices.delete(i);if(dot)dot.style.color='#3d3268';}});selAllBtn.textContent=allOn?'✓ All':'✗ None';updateDelExecuteBtn();};}
            updateDelExecuteBtn();showDelContextMenu(mp);updateStatus(`🗑️ ${deleteCandidates.length} eligible feature(s)${ineligible>0?` · ${ineligible} skipped (no ${DELETE_FIELD} field)`:''}.`);
        }

        async function executeDelete() {
            const toDelete=deleteCandidates.filter((_,i)=>deleteSelectedIndices.has(i));if(!toDelete.length||deleteProcessing)return;
            deleteProcessing=true;delCtxMenu.querySelector('#delCtxExecute').disabled=true;delCtxMenu.querySelector('#delCtxCancel').disabled=true;updateStatus('🗑️ Flagging features…');
            let ok=0,fail=0;for(const c of toDelete){const fieldName=getDeleteFieldName(c.layer);if(!fieldName){fail++;continue;}try{const upd=c.feature.clone();upd.attributes[fieldName]='YES';await c.layer.applyEdits({updateFeatures:[upd]});ok++;}catch(e){console.error(`executeDelete error on ${c.layerConfig.name}:`,e);fail++;}}
            const names=[...new Set(toDelete.slice(0,ok).map(c=>c.layerConfig.name))];updateStatus(ok?`✅ Flagged ${ok} feature(s) as deleted: ${names.join(', ')}${fail?` · ${fail} failed`:''}.`:`❌ All ${fail} flag(s) failed.`);
            ...
            hideDelContextMenu();clearPickerHoverHighlight();deleteCandidates=[];deleteSelectedIndices.clear();setTimeout(()=>{if(deleteMode)updateStatus('🗑️ Delete mode active. Click any feature to flag it.');},3000);
        }
        ...
        function showFeaturePickerPopup(candidates,pageX,pageY){
            ...
            document.body.appendChild(popup);setTimeout(()=>{document.addEventListener("click",function outsideClick(e){if(!popup.contains(e.target)&&!delCtxMenu.contains(e.target)){dismissPickerPopup();document.removeEventListener("click",outsideClick);}});},0);
        }
        ...
        async function updateConnectedLines(newPt) {
            const byLayer=new Map(),cacheUpdates=[];
            for(const info of connectedFeatures){const orig=originalGeometries.get(info.feature.attributes.objectid);if(!orig?.clone)continue;const newGeom=orig.clone();newGeom.paths[info.connection.pathIndex][info.connection.pointIndex]=[newPt.x,newPt.y];const upd=info.feature.clone();upd.geometry=newGeom;upd.attributes.calculated_length=geodeticLength(newGeom);const lid=layerKey(info.layer,info.layerConfig);if(!byLayer.has(lid))byLayer.set(lid,{layer:info.layer,features:[]});byLayer.get(lid).features.push(upd);cacheUpdates.push({lid,oid:getOid(info.feature),geom:newGeom});}
            for(const{layer,features}of byLayer.values())try{if(layer.applyEdits)await layer.applyEdits({updateFeatures:features});}catch(e){console.error('updateConnectedLines batch error:',e);}
        ...
        async function addVertexToLine(event) {
            const sp={x:event.x,y:event.y},mp=mapView.toMap(sp);updateStatus("Adding vertex to line...");
            try{const lines=await findCoincidentLinesForVertexCreation(sp,mp);if(!lines.length){updateStatus("❌ No lines found to add vertex to.");return;}const updates=[],lockedOid=lockedFeature?.featureType==='line'?getOid(lockedFeature.feature):null;
            for(const li of lines){try{const newPaths=clonePaths(li.feature.geometry);newPaths[li.segmentInfo.pathIndex].splice(li.segmentInfo.insertIndex,0,[li.segmentInfo.point.x,li.segmentInfo.point.y]);const newGeom=buildPolyline(li.feature.geometry,newPaths),upd=li.feature.clone();upd.geometry=newGeom;upd.attributes.calculated_length=geodeticLength(newGeom);updates.push({layer:li.layer,feature:upd,layerName:li.layerConfig.name,newGeom,oid:getOid(li.feature),lid:layerKey(li.layer,li.layerConfig)});}catch(e){console.error(`addVertexToLine on ${li.layerConfig.name}:`,e);}}
            if(!updates.length){updateStatus("❌ No vertices could be added.");return;}const byLayer=new Map();for(const u of updates){if(!byLayer.has(u.lid))byLayer.set(u.lid,{layer:u.layer,updates:[]});byLayer.get(u.lid).updates.push(u);}for(const{layer,updates:batch}of byLayer.values())if(layer.applyEdits)await layer.applyEdits({updateFeatures:batch.map(u=>u.feature)});for(const u of updates){updateVertexCacheGeom(u.lid,u.oid,u.newGeom);if(lockedOid!=null&&u.oid===lockedOid)syncLockedFeature(u.newGeom);}
            updateStatus(`✅ Added vertex to ${updates.length} line(s): ${[...new Set(updates.map(u=>u.layerName))].join(", ")}!`);if(vertexHighlightActive)scheduleHighlightRefresh();setTimeout(()=>updateStatus(lockedReadyStatus()),3000);}catch(e){console.error("addVertexToLine error:",e);updateStatus("❌ Error adding vertex.");}
        }

        async function deleteVertexFromLine(event) {
            const sp={x:event.x,y:event.y};updateStatus("Deleting vertex from line...");
            try{const results=await findCoincidentLineVertices(sp);if(!results.length){updateStatus("❌ No line vertex found to delete.");return;}const updates=[],lockedOid=lockedFeature?.featureType==='line'?getOid(lockedFeature.feature):null;
            for(const li of results){try{const srcGeom=li.feature.geometry;if(!srcGeom?.paths)continue;const newPaths=clonePaths(srcGeom),totalVertices=newPaths.reduce((s,p)=>s+p.length,0);if(totalVertices<=2){console.log(`Skipping ${li.layerConfig.name}: only ${totalVertices} vertices.`);continue;}const path=newPaths[li.vertex.pathIndex];if(!path||path.length<=2)continue;path.splice(li.vertex.pointIndex,1);const newGeom=buildPolyline(srcGeom,newPaths),upd=li.feature.clone();upd.geometry=newGeom;upd.attributes.calculated_length=geodeticLength(newGeom);updates.push({layer:li.layer,feature:upd,layerName:li.layerConfig.name,newGeom,oid:getOid(li.feature),lid:layerKey(li.layer,li.layerConfig)});}catch(e){console.error("deleteVertexFromLine prep error:",e);}}
            if(!updates.length){updateStatus("❌ No vertices deleted (lines with only 2 vertices cannot be reduced further).");return;}const byLayer=new Map();for(const u of updates){if(!byLayer.has(u.lid))byLayer.set(u.lid,{layer:u.layer,updates:[]});byLayer.get(u.lid).updates.push(u);}for(const{layer,updates:batch}of byLayer.values())if(layer.applyEdits)await layer.applyEdits({updateFeatures:batch.map(u=>u.feature)});for(const u of updates){updateVertexCacheGeom(u.lid,u.oid,u.newGeom);if(lockedOid!=null&&u.oid===lockedOid)syncLockedFeature(u.newGeom);}
            updateStatus(`✅ Deleted vertex from ${updates.length} line(s): ${[...new Set(updates.map(u=>u.layerName))].join(", ")}!`);if(vertexHighlightActive)scheduleHighlightRefresh();setTimeout(()=>updateStatus(lockedReadyStatus()),3000);}catch(e){console.error("deleteVertexFromLine error:",e);updateStatus("❌ Error deleting vertex.");}
        }
        ...
        async function handleMoveToDestination(event) {
            ...
            try{
                if(currentMode==="point"){
                    ...
                    const excludeOids=new Set([getOid(selectedFeature),...colocatedPoints.map(p=>getOid(p.feature))].filter(Boolean)),snapInfo=snappingEnabled?await findSnapTarget(dst,excludeOids):null;if(snapInfo)dst=toTypedPoint(snapInfo.geometry,mapView.spatialReference);
        ...
        async function handleClick(event) {
            if (!toolActive) return;
            if (event.button !== 0) return;  // ignore right-click and middle-click — handled separately
        ...
        function computePathArrows(path, sr) {
            if(!path||path.length<2)return[];let totalLen=0;const segLens=[];for(let i=0;i<path.length-1;i++){const dx=path[i+1][0]-path[i][0],dy=path[i+1][1]-path[i][1];segLens.push(Math.sqrt(dx*dx+dy*dy));totalLen+=segLens[segLens.length-1];}if(totalLen===0)return[];const n=Math.max(ARROW_MIN,Math.min(ARROW_MAX,Math.floor(totalLen/ARROW_SPACING))),labelShift=ARROW_LABEL_OFFSET_PX*(mapView.resolution||1),arrows=[];
            for(let a=0;a<n;a++){const target=((a+0.5)/n)*totalLen;let dist=0;for(let i=0;i<path.length-1;i++){if(dist+segLens[i]>=target||i===path.length-2){const t=segLens[i]>0?Math.min(1,(target-dist)/segLens[i]):0,dx=path[i+1][0]-path[i][0],dy=path[i+1][1]-path[i][1],bearing=Math.atan2(dx,dy)*180/Math.PI,bRad=bearing*Math.PI/180;arrows.push({x:path[i][0]+t*dx+Math.sin(bRad)*labelShift,y:path[i][1]+t*dy+Math.cos(bRad)*labelShift,bearing,spatialReference:sr});break;}dist+=segLens[i];}}
            return arrows;
        }

        function applyCoincidentOffset(arrows) {
            const offsetDist=(mapView.resolution||1)*10,groupThresh=offsetDist*2,SAME_DIR_DEG=30,processed=new Set();
        ...
        async function renderVertexHighlights() {
            if(!vertexHighlightActive)return;await ensureGraphicClasses();if(!_Graphic||!_GraphicsLayer){updateStatus('❌ Could not load graphic classes.');return;}if(!vertexHighlightLayer){vertexHighlightLayer=new _GraphicsLayer({listMode:"hide"});mapView.map.add(vertexHighlightLayer);}vertexHighlightLayer.removeAll();
            let totalVtx=0;const allArrows=[],sr=mapView.spatialReference;
            const processGeom=geom=>{if(!geom?.paths)return;for(const path of geom.paths){for(let i=0;i<path.length;i++){vertexHighlightLayer.add(makeVertexGraphic(path[i][0],path[i][1],geom.spatialReference,i===0||i===path.length-1));totalVtx++;}if(directionArrowsActive)for(const a of computePathArrows(path,geom.spatialReference||sr))allArrows.push(a);}};
            if(lockedFeature?.featureType==='line'){processGeom(lockedFeature.feature.geometry);}
            else{const currentExtent=mapView.extent;let geomsToRender=[];if(isExtentCovered(currentExtent)){geomsToRender=[...vertexGeomCache.values()].filter(geom=>geomIntersectsExtent(geom,currentExtent));}else{updateStatus("Loading vertex highlights…");const queryResults=await Promise.all(lineLayers.filter(cfg=>cfg.layer.visible).map(async cfg=>{try{const res=await cfg.layer.queryFeatures({geometry:currentExtent,spatialRelationship:"intersects",returnGeometry:true,outFields:["objectid"],maxRecordCount:500});return{cfg,features:res.features};}catch(e){return{cfg,features:[]};}}));for(const{cfg,features}of queryResults){for(const f of features){const oid=getOid(f),cachedGeom=vertexGeomCache.get(vtxKey(cfg.id,oid)),geom=cachedGeom||f.geometry;updateVertexCacheGeom(cfg.id,oid,geom);geomsToRender.push(geom);}}recordQueriedExtent(currentExtent);}for(const geom of geomsToRender)processGeom(geom);}
            if(directionArrowsActive&&allArrows.length){applyCoincidentOffset(allArrows);for(const a of allArrows)vertexHighlightLayer.add(new _Graphic({geometry:{type:'point',x:a.x,y:a.y,spatialReference:a.spatialReference||sr},symbol:{type:'simple-marker',style:'triangle',color:[167,139,250,210],size:11,outline:{color:[255,255,255,160],width:1},angle:a.bearing}}));}
            const scope=lockedFeature?.featureType==='line'?`locked (${lockedFeature.layerConfig.name})`:`${lineLayers.filter(l=>l.layer.visible).length} line layer(s)`,cached=isExtentCovered(mapView.extent)&&lockedFeature?.featureType!=='line'?' (cached)':'';
        ...
        // Clear just the anchor (keep line selected)
        function clearArcAnchor() {
            arcAnchorMode = false;
            arcAnchorPt = null;
            arcAnchorSegInfo = null;
            arcActiveSide = 'left';
            clearArcAnchorGraphic();
            clearArcPreview();
        }

        function resetArcState() {
            arcWaitingForMidpoint = false;
            clearArcAnchor();
            arcSelectedLines = []; arcStartPt = null; arcEndPt = null;
            clearArcSelectionHighlight(); clearArcPreview();
        }
        ...
        async function handleArcClick(event) {
            ...
            if (!arcWaitingForMidpoint) {
                ...
                if (lockedFeature?.featureType === 'line') {
                    lines = [{ feature: lockedFeature.feature, layer: lockedFeature.layer, layerConfig: lockedFeature.layerConfig }];
                } else {
                    lines = await findLinesAtClick(sp, mp);
                }
                ...
                const path = lines[0].feature.geometry.paths[seg.pathIndex];
                const geomSr = lines[0].feature.geometry.spatialReference;
                ...
                arcSelectedLines = lines;
                arcWaitingForMidpoint = true;
                showArcSelectionHighlight(lines);
                ...
                // ── Step 2: apply the arc ─────────────────────────────────────
                clearArcPreview();

                // Determine which endpoints bound the arc based on anchor mode
                const p1 = arcAnchorMode && arcActiveSide === 'right' ? arcAnchorPt : arcStartPt;
                const p2 = arcAnchorMode && arcActiveSide === 'left'  ? arcAnchorPt : arcEndPt;
                ...
                for (const li of arcSelectedLines) {
                    try {
                        ...
                        if (arcAnchorMode) {
                            // Find the equivalent anchor position on this specific line's geometry
                            // (handles coincident lines that may have slightly different vertex counts)
                            const info = findClosestPointOnPath(li.feature.geometry, arcAnchorPt);
                            const builtPath = buildAnchoredArcPath(li.feature.geometry, info, arcPts, arcActiveSide);
                            newPaths[info.pathIdx] = builtPath;
                        } else {
                            // Original full-path replacement: find path whose endpoints best match
                            const paths = li.feature.geometry.paths;
                            let bestPi = 0, bestDist = Infinity;
                            for (let pi = 0; pi < paths.length; pi++) {
                                const p = paths[pi];
                                const d = Math.min(
                                    calcDist(arcStartPt,{x:p[0][0],y:p[0][1]}) + calcDist(arcEndPt,{x:p[p.length-1][0],y:p[p.length-1][1]}),
                                    calcDist(arcEndPt,  {x:p[0][0],y:p[0][1]}) + calcDist(arcStartPt,{x:p[p.length-1][0],y:p[p.length-1][1]})
                                );
                                if (d < bestDist) { bestDist = d; bestPi = pi; }
                            }
                            const p = paths[bestPi];
                            const isReversed = calcDist(arcEndPt,{x:p[0][0],y:p[0][1]}) < calcDist(arcStartPt,{x:p[0][0],y:p[0][1]});
                ...
                for (const { layer, updates: batch } of byLayer.values()) {
                    ...
                    catch(e) { console.error('handleArcClick applyEdits error:', e); }
                }
        ...
        function enableArcMode() {
            ...
            if (deleteMode) disableDeleteMode();
            arcMode = true;
            setActiveModeBtn('arcModeBtn');
            showCtxPanel(null);

            // Suppress browser context menu while a line is selected in arc mode
            arcContextMenuHandler = e => { if (arcWaitingForMidpoint) e.preventDefault(); };
            mapView.container.addEventListener('contextmenu', arcContextMenuHandler);

            // Right-click (pointer-down button 2) plants or repositions the anchor
            arcRightClickHandler = mapView.on('pointer-down', e => {
                if (e.button !== 2 || !arcWaitingForMidpoint || arcSelectedLines.length === 0) return;
                const mp = mapView.toMap({ x: e.x, y: e.y });
                const geom = arcSelectedLines[0].feature.geometry;
                const info = findClosestPointOnPath(geom, mp);
                arcAnchorPt = { x: info.point.x, y: info.point.y, spatialReference: geom.spatialReference };
                arcAnchorSegInfo = info;
                arcAnchorMode = true;
                showArcAnchorGraphic(arcAnchorPt);
                updateStatus('⌒ Anchor set ◆ — hover either side to preview the curve, then left-click to apply. Right-click to reposition. ESC to clear anchor.');
            });

            // Live preview on pointer-move
            arcMoveHandler = mapView.on('pointer-move', e => {
                if (!arcWaitingForMidpoint || !arcStartPt || !arcEndPt) return;
                const mp = mapView.toMap({ x: e.x, y: e.y });
                const sr = arcStartPt.spatialReference || mapView.spatialReference;

                if (arcAnchorMode && arcAnchorPt && arcAnchorSegInfo) {
                    // ── Anchor mode: preview one side, show original geometry on the other ──
                    const geom = arcSelectedLines[0].feature.geometry;
                    arcActiveSide = getArcSide(geom, arcAnchorSegInfo, mp);

                    let previewPts;
                    if (arcActiveSide === 'left') {
                        // Arc from path-start → anchor; original verts from anchor → path-end
                        const circle = circumscribedCircle(arcStartPt, mp, arcAnchorPt);
                        if (!circle) { clearArcPreview(); return; }
                        const arcPts = generateArcPoints(arcStartPt, arcAnchorPt, mp, circle, sr);
                        const path = geom.paths[arcAnchorSegInfo.pathIdx];
                        const tail = path.slice(arcAnchorSegInfo.segIdx + 1).map(v => [v[0], v[1]]);
                        previewPts = [...arcPts, ...tail];
                    } else {
                        // Original verts from path-start → anchor; arc from anchor → path-end
                        const circle = circumscribedCircle(arcAnchorPt, mp, arcEndPt);
                        if (!circle) { clearArcPreview(); return; }
                        const arcPts = generateArcPoints(arcAnchorPt, arcEndPt, mp, circle, sr);
                        const path = geom.paths[arcAnchorSegInfo.pathIdx];
                        const head = path.slice(0, arcAnchorSegInfo.segIdx + 1).map(v => [v[0], v[1]]);
                        previewPts = [...head, [arcAnchorPt.x, arcAnchorPt.y], ...arcPts.slice(1)];
                    }
                    showArcPreview(previewPts, sr);

                } else {
                    // ── Standard mode: full-path arc preview ──
                    const circle = circumscribedCircle(arcStartPt, mp, arcEndPt);
                    if (!circle) { clearArcPreview(); return; }
                    showArcPreview(generateArcPoints(arcStartPt, arcEndPt, mp, circle, sr), sr);
                }
            });

            if (toolActive) updateStatus('⌒ Arc Fit active — click a line to select it.');
        }

        function disableArcMode() {
            arcMode = false;
            resetArcState();
            if (arcMoveHandler)        { arcMoveHandler.remove();        arcMoveHandler = null; }
            if (arcRightClickHandler)  { arcRightClickHandler.remove();  arcRightClickHandler = null; }
            if (arcContextMenuHandler) { mapView.container.removeEventListener('contextmenu', arcContextMenuHandler); arcContextMenuHandler = null; }
