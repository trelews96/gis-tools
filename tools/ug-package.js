...
async function main(){
  ...
  styleEl.textContent=`
    ...
    #pkgT .grid3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:0 8px;}
  ...
  function closeAllDrops(){if(activePortal){activePortal.remove();activePortal=null;}box.querySelectorAll('.ss-btn.open').forEach(b=>{b.classList.remove('open');b.style.borderRadius='';});}
  ...
  function mkToggle(id,label,isOn){return`<label class="tog" onclick="window.__pkgToggle('${id}')"><div class="tog-track ${isOn?'on':''}" id="ttrack_${id}"><div class="tog-thumb"></div></div><span class="tog-lbl">${label}</span></label>`;}
  ...
  function mkDate(domId,cur=''){return`<input type="date" id="${domId}" value="${cur}">`;}
  ...
  // ── Alignment ─────────────────────────────────────────────────────────
  function nearestCleanAngle(bearing){const snapped=Math.round(bearing/45)*45%360;let diff=Math.abs(bearing-snapped);if(diff>180)diff=360-diff;return diff<=snapThreshDeg?snapped:null;}
  function resolvePoint(rawPt){
    ...
    if(bearingLock&&lockedBearing!==null){const proj=Geo.projectOnBearing(origin,lockedBearing,rawPt);return{commitPt:proj,snapPt:proj,snapping:true,snapBearing:lockedBearing,isLocked:true,isSoft:false};}
    if(snapMode!=='off'){const clean=nearestCleanAngle(rawBearing);if(clean!==null){const proj=Geo.projectOnBearing(origin,clean,rawPt);const isSoft=snapMode==='soft';return{commitPt:isSoft?rawPt:proj,snapPt:proj,snapping:true,snapBearing:clean,isLocked:false,isSoft};}}
    return{commitPt:rawPt,snapPt:null,snapping:false,snapBearing:null,isLocked:false,isSoft:false};
  }

  // ── Segment Builder ───────────────────────────────────────────────────
  function buildSegments(pts){
    if(pts.length<2)return[];const segs=[];let cur=[pts[0].pt];
    for(let i=1;i<pts.length;i++){cur.push(pts[i].pt);if(!pts[i].noVault&&i<pts.length-1){segs.push(cur);cur=[pts[i].pt];}}
    if(cur.length>=2)segs.push(cur);return segs;
  }

  // ── Arc Math ──────────────────────────────────────────────────────────
  function circumcenter(p1,p2,p3){const ax=p1.x,ay=p1.y,bx=p2.x,by=p2.y,cx=p3.x,cy=p3.y,D=2*(ax*(by-cy)+bx*(cy-ay)+cx*(ay-by));if(Math.abs(D)<1e-6)return null;return{x:((ax*ax+ay*ay)*(by-cy)+(bx*bx+by*by)*(cy-ay)+(cx*cx+cy*cy)*(ay-by))/D,y:((ax*ax+ay*ay)*(cx-bx)+(bx*bx+by*by)*(ax-cx)+(cx*cx+cy*cy)*(bx-ax))/D};}
  function generateArcWaypoints(p1,pMid,p3,densityFt){
    const sr=p1.spatialReference,cc=circumcenter(p1,pMid,p3);
    if(!cc){const N=Math.max(1,Math.round(Math.hypot(p3.x-p1.x,p3.y-p1.y)/(densityFt*0.3048)));const res=[];for(let i=1;i<=N;i++){const t=i/N;res.push({x:p1.x+(p3.x-p1.x)*t,y:p1.y+(p3.y-p1.y)*t,spatialReference:sr});}return res;}
    const R=Math.hypot(p1.x-cc.x,p1.y-cc.y),a1=Math.atan2(p1.y-cc.y,p1.x-cc.x),a3=Math.atan2(p3.y-cc.y,p3.x-cc.x);
    const cross=(pMid.x-p1.x)*(p3.y-p1.y)-(pMid.y-p1.y)*(p3.x-p1.x);
    let sweep=a3-a1;if(cross>0&&sweep<0)sweep+=2*Math.PI;if(cross<=0&&sweep>0)sweep-=2*Math.PI;
    const N=Math.max(2,Math.round(Math.abs(sweep)*R/(densityFt*0.3048)));const res=[];
    for(let i=1;i<=N;i++){const ang=a1+(sweep*i/N);res.push({x:cc.x+R*Math.cos(ang),y:cc.y+R*Math.sin(ang),spatialReference:sr});}return res;
  }
  ...
  function addMarker(pt,idx,isFree=false){
    ...
    const sym=isFree?{type:'simple-marker',style:'x',color:[137,220,235,0.9],size:'11px',outline:{color:[137,220,235,1],width:2}}:{type:'simple-marker',style:'circle',color:[203,166,247,0.92],size:'13px',outline:{color:[255,255,255,0.9],width:2}};
  ...
  function refreshLine(){
    if(!gl||!window.__pkgGr)return;rmTag('line');if(pts.length<2)return;
  ...
  function updatePreview(rawMapPt){
    ...
    const origin=pts.length?pts[pts.length-1].pt:null;if(!origin)return;
    const res=resolvePoint(rawMapPt);pendingPt=res.commitPt;
    if(bearingLock&&lockedBearing!==null){const far=500000,ah=Geo.extendAlong(origin,lockedBearing,far),bh=Geo.extendAlong(origin,(lockedBearing+180)%360,far);gl.add(new window.__pkgGr({geometry:{type:'polyline',paths:[[[bh.x,bh.y],[ah.x,ah.y]]],spatialReference:origin.spatialReference},symbol:{type:'simple-line',color:[166,227,161,0.22],width:1,style:'long-dash'},attributes:{_pkg:'preview-ray'}}));}
    if(res.isSoft&&res.snapPt){gl.add(new window.__pkgGr({geometry:{type:'polyline',paths:[[[origin.x,origin.y],[res.snapPt.x,res.snapPt.y]]],spatialReference:origin.spatialReference},symbol:{type:'simple-line',color:[250,179,135,0.35],width:1.5,style:'long-dash'},attributes:{_pkg:'preview-ghost'}}));gl.add(new window.__pkgGr({geometry:res.snapPt,symbol:{type:'simple-marker',style:'circle',color:[250,179,135,0.25],size:'10px',outline:{color:[250,179,135,0.7],width:1.5}},attributes:{_pkg:'preview-ghost'}}));}
    ...
    const rawB=Geo.bearing(origin,rawMapPt);updateBearingBar(rawB,res);
  }
  ...
  function updateBearingBar(rawBearing,res){
    ...
    if(ind){if(res.isLocked){ind.textContent=`🔒 ${lockedBearing.toFixed(0)}°`;ind.style.background='#142d1d';ind.style.color='#a6e3a1';}else if(res.snapping&&res.isSoft){ind.textContent=`⊕ Guide ${res.snapBearing}°`;ind.style.background='#2d1a0a';ind.style.color='#fab387';}else if(res.snapping){ind.textContent=`⊕ Snap ${res.snapBearing}°`;ind.style.background='#3d2200';ind.style.color='#f9a836';}else{ind.textContent='Free';ind.style.background='#313244';ind.style.color='#585b70';}}
  }
  ...
  // ── Templates ─────────────────────────────────────────────────────────
  const Tmpl={all(){return JSON.parse(localStorage.getItem(TMPL_KEY)||'{}');},save(name,data){const t=this.all();t[name]={name,created:new Date().toISOString(),data};localStorage.setItem(TMPL_KEY,JSON.stringify(t));},del(name){const t=this.all();delete t[name];localStorage.setItem(TMPL_KEY,JSON.stringify(t));}};
  ...
  function tmplCardHTML(t){
    ...
    return`<div class="tcard"><div class="tcard-hdr"><span class="tcard-name">${t.name}${isCurrent?'<span class="current-tmpl-tag">current</span>':''}</span><span class="tcard-date">${new Date(t.created).toLocaleDateString()}</span></div>
      <div class="tcard-row"><span>🔵</span><span>${cdStr}</span></div><div class="tcard-row"><span>🟣</span><span>${fbStr} ${d.fiberSplit===false?'(single span)':'(split per seg)'}</span></div><div class="tcard-row"><span>🏗️</span><span>${vaultStr}</span></div>
      <div class="row" style="margin-top:6px;gap:5px"><button class="btn btn-p btn-sm" onclick="window.__pkgLoadTmpl('${t.name}')">Load</button><button class="btn btn-r btn-sm" onclick="window.__pkgDelTmpl('${t.name}')">Delete</button></div></div>`;
  }
  ...
  // ── Pick from Map ─────────────────────────────────────────────────────
  function startPick(){
    if(pickH){pickH.remove();pickH=null;}mv.container.style.cursor='crosshair';
  ...
  // Build a clean attrs object from an override block, converting date string → epoch ms
  ...
  // ── Shared override fields section HTML ───────────────────────────────
  function ovrSection(idPrefix,state){
    return`<div class="ovr-sec">
      <div class="ovr-title">Optional — Supervisor / Crew / Date / Status Override</div>
      <div class="grid2">
  ...
  function renderPlace(){
    const vaultCt=pts.filter(p=>!p.noVault).length,freeCt=pts.filter(p=>p.noVault).length;
    const segs=buildSegments(pts).length,len=pts.length>1?Geo.totalLen(pts.map(p=>p.pt)):0;
    ...
    return`
      ...
      ${active&&pts.length?`<div class="bearing-bar" id="bearing-bar">
        <div><div style="font-size:9px;color:#6c7086;margin-bottom:1px">BEARING</div><div class="bearing-val" id="bearing-val">—</div></div>
        <div style="flex:1"><span class="bearing-snap-ind" id="bearing-snap-ind" style="background:#313244;color:#585b70">Free</span></div>
        <div style="font-size:9px;color:#585b70;text-align:right">${snapBtnLabel()}<br>${bearingLock?'🔒 Lock ON':'Lock OFF'}</div>
      </div>`:``}
      ${arcMode?`<div class="arc-guide">
        ...
        <div><div style="font-weight:700">⌒ Arc — click the <em>${arcLabels[arcPts.length]||'complete'}</em></div>
          <div class="arc-density-row">Spacing: <input type="number" id="arc-density-inp" value="${arcDensity}" min="10" max="500"> ft &nbsp;<button class="btn btn-r btn-sm" onclick="window.__pkgCancelArc()">Cancel</button></div>
        </div>
      </div>`:``}
      <div class="sec"><div class="sec-hdr" style="cursor:default">Placed Points</div>
        <div class="sec-body" style="padding:6px"><div class="plist" id="plist">
          ${pts.length?pts.map((p,i)=>`<div class="pitem"><span>${p.noVault?'✕':'📍'} ${p.noVault?'Free Pt':'Vault'} ${i+1}</span><span style="color:#6c7086;font-size:9px">${p.pt.x.toFixed(0)}, ${p.pt.y.toFixed(0)}</span>${i>0?`<span style="color:#585b70;font-size:9px">+${Geo.dist(pts[i-1].pt,p.pt).toLocaleString()} ft</span>`:'<span></span>'}</div>`).join(''):`<div class="empty">Enable placement and click the map.</div>`}
        </div></div></div>
      ...
      <div class="sec collapsed" style="margin-top:8px">
        ...
        <div style="margin-top:7px;font-size:9px;color:#585b70">Soft snap shows alignment guide without forcing position. Hard snap locks click to nearest 45°.</div>
        </div>
      </div>`;
  }
  ...
  function renderReview(){
    ...
    const sCt=segCt*conduits.length,fCt=fiberSplit?segCt*fibers.length:fibers.length;
    ...
    return`
      <div class="sec"><div class="sec-hdr" style="cursor:default">Package Summary — ${total} features</div>
        <div class="sec-body" style="padding:6px"><table class="rvtbl">
          ...
          <tr><td>Spans</td><td><span class="badge bp">${segCt} × ${conduits.length} = ${sCt}</span></td></tr>
          <tr><td>Fiber Cables</td><td><span class="badge bp">${fiberSplit?`${segCt} × ${fibers.length} = ${fCt}`:`${fibers.length} (full span)`}</span></td></tr>
  ...
  function bind(){
    ...
    if(step==='layers'){
      on('btn-to-setup','click',()=>{saveVCfg();saveConduits();saveFibers();saveSpanOverride();saveFiberOverride();step='setup';render();});
      on('btn-to-place','click',()=>{saveVCfg();saveConduits();saveFibers();saveSpanOverride();saveFiberOverride();step='place';render();initGL();});
    ...
    if(step==='place'){
      ...
      on('btn-clear','click',()=>{pts=[];clearGL();waypointMode=false;cancelArc();bearingLock=false;lockedBearing=null;refreshPlaceUI();});
    }
    if(step==='review'){on('btn-to-place','click',()=>{step='place';render();});on('btn-commit','click',commitPackage);}
  }

  // ── Mode toggles ──────────────────────────────────────────────────────
  function toggleFreePoint(){if(arcMode){arcMode=false;cancelArc();}waypointMode=!waypointMode;const btn=qs('#btn-waypoint');if(btn){btn.classList.toggle('btn-active',waypointMode);btn.textContent=waypointMode?'✕ Free Pt ON':'✕ Free Pt';}setStatus(waypointMode?'Free point mode ON.':'Free point mode OFF.','info');}
  ...
  function toggleBearingLock(){
    ...
    else{const vaults=pts.filter(p=>!p.noVault);const p1=vaults.length>=2?vaults[vaults.length-2].pt:pts.length>=2?pts[pts.length-2].pt:null;const p2=vaults.length>=1?vaults[vaults.length-1].pt:pts.length>=1?pts[pts.length-1].pt:null;if(!p1||!p2){setStatus('Need at least 2 points to lock a bearing.','warn');return;}lockedBearing=Geo.bearing(p1,p2);bearingLock=true;setStatus(`🔒 Bearing locked to ${lockedBearing.toFixed(1)}°`,'success');}
    render();
  }
  ...
  function enableTool(){
    ...
    setStatus('Placement active — click map to place vaults.');render();
  }
  function disableTool(){
    active=false;if(clickH){clickH.remove();clickH=null;}if(moveH){moveH.remove();moveH=null;}
    if(keyH){document.removeEventListener('keydown',keyH);keyH=null;}
    if(mv?.container)mv.container.style.cursor='default';removeSnapRing();clearPreview();
  ...
  async function handleClick(evt){
    ...
    if(arcMode){
      ...
      const pt=mv.toMap({x:evt.x,y:evt.y});arcPts.push(pt);addArcGuideMarker(pt,arcPts.length);
      if(arcPts.length<3){setStatus(`Arc: click the ${['','midpoint along curve','end of curve'][arcPts.length]}…`,'warn');render();return;}
      const wps=generateArcWaypoints(arcPts[0],arcPts[1],arcPts[2],arcDensity);clearArcGuides();
      for(const wp of wps){pts.push({pt:wp,noVault:true});addMarker(wp,pts.length-1,true);}
      cancelArc();refreshPlaceUI();render();setStatus(`⌒ Arc complete — ${wps.length} free points added.`,'success');return;
    }
  ...
  function handleKey(e){
    if(!active)return;if(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA')return;
    ...
    if(k==='Z'||e.key==='Escape'){e.preventDefault();if(arcMode){cancelArc();render();setStatus('Arc cancelled.');return;}undoLast();return;}
    if(e.key==='Enter'&&pts.length>=2&&!arcMode){disableTool();step='review';render();}
  }

  function undoLast(){
    if(arcMode&&arcPts.length){arcPts.pop();clearArcGuides();arcPts.forEach((p,i)=>addArcGuideMarker(p,i+1));render();setStatus('Arc point removed.','warn');return;}
    if(!pts.length)return;pts.pop();removeLastMarker();clearPreview();
    if(bearingLock){const vaults=pts.filter(p=>!p.noVault);if(vaults.length>=2)lockedBearing=Geo.bearing(vaults[vaults.length-2].pt,vaults[vaults.length-1].pt);else{bearingLock=false;lockedBearing=null;}}
    refreshPlaceUI();setStatus(pts.length?`Point ${pts.length+1} removed.`:'All points cleared.');
  }

  function refreshPlaceUI(){
    const listEl=qs('#plist');
    if(listEl)listEl.innerHTML=pts.length?pts.map((p,i)=>`<div class="pitem"><span>${p.noVault?'✕':'📍'} ${p.noVault?'Free Pt':'Vault'} ${i+1}</span><span style="color:#6c7086;font-size:9px">${p.pt.x.toFixed(0)}, ${p.pt.y.toFixed(0)}</span>${i>0?`<span style="color:#585b70;font-size:9px">+${Geo.dist(pts[i-1].pt,p.pt).toLocaleString()} ft</span>`:'<span></span>'}</div>`).join(''):`<div class="empty">Enable placement and click the map.</div>`;
    const undo=qs('#btn-undo'),clr=qs('#btn-clear'),rev=qs('#btn-to-review'),lock=qs('#btn-lock');
    if(undo)undo.disabled=!pts.length;if(clr)clr.disabled=!pts.length;if(rev)rev.disabled=pts.length<2;if(lock)lock.disabled=pts.length<2;
  }
  ...
  async function commitPackage(){
    ...
    try{
      ...
      // Per-layer option sets (filter out empty strings)
      const vaultOpts=Object.fromEntries(Object.entries(vCfg).filter(([k,v])=>v!==''&&!['workflow_status_ovr'].includes(k)));
      ...
      for(let ri=0;ri<conduits.length;ri++){
        ...
        for(let si=0;si<segments.length;si++){
          ...
          try{const id=await createFeat(sl,geom,{...base,...spanOpts,...spanOvrAttrs,calculated_length:Geo.totalLen(segPts)});log.spans.push(id);}
      ...
      for(let ri=0;ri<fibers.length;ri++){
        ...
        if(fiberSplit){
          for(let si=0;si<segments.length;si++){
            ...
            const geom={type:'polyline',paths:[segPts.map(p=>[p.x,p.y])],spatialReference:segPts[0].spatialReference};
            try{const id=await createFeat(cl,geom,{...base,...fibOpts,...fiberOvrAttrs,cable_name:name,calculated_length:Geo.totalLen(segPts)});log.fibers.push(id);}
          ...
          try{const id=await createFeat(cl,geom,{...base,...fibOpts,...fiberOvrAttrs,cable_name:name,calculated_length:Geo.totalLen(pts.map(p=>p.pt))});log.fibers.push(id);}
  ...
  window.__pkgNewPkg=()=>{pts=[];waypointMode=false;cancelArc();bearingLock=false;lockedBearing=null;clearGL();step='place';render();};
