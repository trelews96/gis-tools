...
(function () {
  try {
    ...
    let pendingGigAttachment = null; // { blob, filename } — uploaded after GIG creation
    ...
    css.textContent = `
      ...
      /* Markup toggle button highlight */
    ...
    lb.innerHTML = `
      <div class="lb-toolbar">
        ...
        <div class="lb-zoom-btns">
          ...
          <button id="lbMkToggle" title="Toggle markup tools (M)" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:4px 11px;border-radius:5px;cursor:pointer;font-size:14px;font-weight:700;transition:background .15s;">✏️</button>
        </div>
      ...
      <div class="lb-stage grab" id="lbStage">
        ...
        <!-- Markup canvas sits exactly over lbImg -->
      ...
      <!-- Markup toolbar (bottom-center) -->
      <div id="lbMkBar">
        <!-- Tools -->
        <button class="mk-btn sel" id="mkPen"    title="Freehand pen (P)">✏️</button>
        <button class="mk-btn"     id="mkCircle" title="Circle / oval (C)">⭕</button>
        <button class="mk-btn"     id="mkArrow"  title="Arrow (A)" style="font-size:16px;">↗</button>
        <button class="mk-btn"     id="mkText"   title="Text callout (T)" style="font-weight:700;font-size:14px;">T</button>
        <div class="mk-sep"></div>
        <!-- Stroke width -->
        <input type="range" class="mk-lw" id="mkWidth" min="2" max="12" value="3" title="Stroke width">
        <div class="mk-sep"></div>
        <!-- Colors -->
        <div class="mk-dot sel" data-c="#ff3b3b" style="background:#ff3b3b;" title="Red"></div>
        <div class="mk-dot"     data-c="#ffcc00" style="background:#ffcc00;" title="Yellow"></div>
        <div class="mk-dot"     data-c="#00cc55" style="background:#00cc55;" title="Green"></div>
        <div class="mk-dot"     data-c="#3b9fff" style="background:#3b9fff;" title="Blue"></div>
        <div class="mk-dot"     data-c="#ffffff" style="background:#fff;"    title="White"></div>
        <div class="mk-sep"></div>
        <!-- Undo / clear -->
        <button class="mk-btn" id="mkUndo"  title="Undo (Ctrl+Z)">↩</button>
        <button class="mk-btn" id="mkClear" title="Clear all">🗑</button>
        <div class="mk-sep"></div>
        <button class="mk-queue" id="mkQueue" disabled title="Save marked-up image as attachment on next GIG submit">📎 Queue for GIG</button>
      </div>
    ...
    function updateFilterBadge() {
      let n=0;
      if(selectedWOs.size)n++; if(selectedPOs.size)n++; if(selectedJobs.size)n++;
      ...
      if (!forceRefresh) {
        try {
          ...
          if (raw) {
            ...
            if ((Date.now()-c.ts) < OPTS_CACHE_TTL) {
              applyFilterOptions(c);
              const age = Math.round((Date.now()-c.ts)/60000);
              $('#cacheBadge').style.display='inline-flex';
              $('#cacheBadge').textContent=`⚡ cached ${age<2?'just now':age<60?age+'m ago':Math.round(age/60)+'h ago'}`;
    ...
    async function fetchFilterOptions() {
      const [gigTypesData] = await Promise.all([
        (async()=>{
          ...
          if(!fld?.domain?.codedValues) throw new Error('gig_type domain not found');
        ...
        (async()=>{
          ...
          await Promise.all(layers.items.map(async lyr=>{
            try {
              ...
              await lyr.load();
              await Promise.all([
                {name:'workorder_id',map:woAcc},{name:'purchase_order_id',map:poAcc},{name:'job_number',map:jobAcc}
              ].map(async({name,map})=>{
                const fld=lyr.fields.find(f=>f.name.toLowerCase()===name);
                if(!fld)return;
                const dom=new Map();
                if(fld.domain?.type==='coded-value')fld.domain.codedValues.forEach(cv=>dom.set(cv.code,cv.name));
                const q=lyr.createQuery();q.where='1=1';q.returnDistinctValues=true;q.outFields=[fld.name];q.returnGeometry=false;
                const res=await lyr.queryFeatures(q);
                res.features.forEach(f=>{const c=f.attributes[fld.name];if(c==null||c==='')return;map.set(c,dom.get(c)??c);});
              }));
            }catch(_){}
          }));
        })()
    ...
    function setupMultiDropdown(inputSel,dropSel,pillSel,options,selectedSet,nameMap) {
      ...
      const render=(filter='')=>{
        ...
        const hdr=document.createElement('div');hdr.className='dd-hdr';
        ...
        if(!hits.length){const none=document.createElement('div');none.className='dd-item';none.style.color='#94a3b8';none.textContent='No matches';dd.appendChild(none);}
          ...
          const d=document.createElement('div');d.className='dd-check-row'+(selectedSet.has(o.code)?' is-checked':'');
          const cb=document.createElement('input');cb.type='checkbox';cb.checked=selectedSet.has(o.code);
          const lbl=document.createElement('span');lbl.textContent=o.name;
          d.appendChild(cb);d.appendChild(lbl);
          d.onmousedown=e=>{e.preventDefault();if(selectedSet.has(o.code))selectedSet.delete(o.code);else{selectedSet.add(o.code);nameMap.set(o.code,o.name);}syncDisplay();updateFilterBadge();render(inp.value);};
          dd.appendChild(d);
        });
      };
    ...
    function refreshMultiDisplays() {
      [['#woSearch','#woPill',selectedWOs,woNameMap],['#poSearch','#poPill',selectedPOs,poNameMap],['#jobSearch','#jobPill',selectedJobs,jobNameMap]].forEach(([iS,pS,set,map])=>{
        const inp=$(iS),pill=$(pS);if(!inp)return;
    ...
    box.querySelectorAll('.qd-strip [data-days]').forEach(btn=>{
      btn.onclick=()=>{const d=parseInt(btn.dataset.days),to=new Date(),fr=new Date();fr.setDate(fr.getDate()-d);$('#dateTo').value=to.toISOString().split('T')[0];$('#dateFrom').value=fr.toISOString().split('T')[0];updateFilterBadge();};
    });
    ...
    async function initSketch() {
      ...
      sketchLayer=new GL({listMode:'hide',title:'QC Selection Area'});
      ...
      sketchVM.on('create',async e=>{
        if(e.state==='start'){isDrawing=true;setDrawHint('drawing','Drawing…');}
          ...
          isDrawing=false;let geom=e.graphic.geometry;
          if(geom&&mapView.spatialReference&&!geom.spatialReference?.wkid)geom=Object.assign(Object.create(Object.getPrototypeOf(geom)),geom,{spatialReference:mapView.spatialReference});
          try{const ge=await loadModule('esri/geometry/geometryEngine');const s=ge.simplify(geom);if(s)geom=s;}catch(_){}
          drawnGeometry=geom;setSpatialMode('drawn');setDrawHint('done','✓ Area selected — run query to apply');setStatus('Drawn area ready','ok');
    ...
    function startDraw(tool,drawMode){
      ...
      if(isDrawing)sketchVM.cancel();clearSketchGraphics();sketchVM.create(tool,{mode:drawMode});
    ...
    async function runQuery(){
      try{setStatus('Querying…','busy');$('#btnQuery').disabled=true;
    ...
    async function queryNewFeatures(){
      ...
      for(const lyr of getCandidateLayers().items){
        try{
          if(lyr===sketchLayer)continue;await lyr.load();
          if(!['workflow_stage','workflow_status','gis_id'].every(fn=>lyr.fields.some(f=>f.name.toLowerCase()===fn)))continue;
          const q=lyr.createQuery();q.where=where;q.outFields=['*'];q.returnGeometry=true;applySpatialFilter(q);
          try{q.orderByFields=[`installation_date ${sort.toUpperCase()}`];}catch(_){}
          const res=await lyr.queryFeatures(q);
          if(res.features.length){counts[lyr.title]=res.features.length;res.features.forEach(f=>{const gisId=f.attributes.gis_id||f.attributes.GIS_ID||f.attributes.gisid||'Unknown';qcQueue.push({layer:lyr,feature:f,gisId,type:'new_qc'});});}
    ...
    async function queryClearedGigs(){
      ...
      if(!gigLyr)throw new Error('GIG layer (22100) not found');await gigLyr.load();
      ...
      const sort=$('#sortOrder').value,q=gigLyr.createQuery();
      q.where=clauses.join(' AND ');q.outFields=['*'];q.returnGeometry=true;applySpatialFilter(q);
    ...
    function setupIssueTypes(){
      ...
      const render=(filter='')=>{
        list.innerHTML='';const lc=filter.toLowerCase();
        ...
        if(!lc&&recentGigTypes.length){
          const rc=new Set(recentGigTypes.map(r=>String(r.code)));
          const recent=hits.filter(gt=>rc.has(String(gt.code))),rest=hits.filter(gt=>!rc.has(String(gt.code)));
    ...
    function renderFeatureInfo(item){
      ...
      if(!isNew&&a.comments){banner.style.display='block';$('#gigCommentsText').textContent=a.comments;}else banner.style.display='none';
    }
    function resetForm(){
      ...
      // Clear pending markup for this feature
    ...
    function gigPointGeom(g){
      ...
      if(g.type==='polyline'&&g.paths?.[0]?.length>1){const path=g.paths[0];let total=0;const segs=path.slice(0,-1).map((pt,i)=>{const dx=path[i+1][0]-pt[0],dy=path[i+1][1]-pt[1],len=Math.sqrt(dx*dx+dy*dy);total+=len;return{x1:pt[0],y1:pt[1],x2:path[i+1][0],y2:path[i+1][1],len,cum:total};});let prev=0,target=total/2;for(const s of segs){if(s.cum>=target){const t=(target-prev)/s.len;return{type:'point',x:s.x1+(s.x2-s.x1)*t,y:s.y1+(s.y2-s.y1)*t,spatialReference:g.spatialReference};}prev=s.cum;}}
    ...
    async function loadAttachments(item){
      $('#attachmentsBar').style.display='none';
      try{
        const lyr=item.layer;await lyr.load();
        ...
        const atts=(res[oid]||[]).filter(a=>a.contentType?.startsWith('image/'));
        if(!atts.length)return;
        $('#attachmentsBar').style.display='block';
        $('#attachCount').textContent=atts.length;
        $('#btnAttachments').onclick=()=>openLightbox(atts);
    ...
    function closeLightbox(){
      if(mkActive)deactivateMarkup();
      lb.classList.remove('open');$lb('#lbImg').src='';
    }
    function showLbImage(){
      ...
      img.onload=()=>{
        $lb('#lbLoading').style.display='none';img.style.display='block';
        if(mkActive)resizeMkCanvas();
      };
      ...
      // Clear strokes when switching images
      mkStrokes=[];mkCurrent=null;
    ...
    // Stage pan/zoom events (disabled during markup)
    ...
    // ── Markup system ─────────────────────────────────────────────────
    mkCanvas = $lb('#lbMkCanvas');
    mkCtx    = mkCanvas.getContext('2d');

    function activateMarkup(){
      mkActive=true;
      // Freeze zoom/pan and reset to 1:1 so canvas aligns perfectly
      lbScale=1;lbPan={x:0,y:0};applyLbTransform();
      resizeMkCanvas();
      mkCanvas.style.display='block';
      $lb('#lbMkBar').classList.add('open');
      $lb('#lbMkToggle').classList.add('on');
      $lb('#lbHint').style.opacity='0';
      lbStage.style.cursor='crosshair';
    }
    function deactivateMarkup(){
      mkActive=false;
      mkCanvas.style.display='none';
      $lb('#lbMkBar').classList.remove('open');
      $lb('#lbMkToggle').classList.remove('on');
      $lb('#lbMkTextWrap').style.display='none';
      $lb('#lbHint').style.opacity='1';
      lbStage.style.cursor='';
    }
    $lb('#lbMkToggle').onclick=()=>{ mkActive?deactivateMarkup():activateMarkup(); };

    function resizeMkCanvas(){
      const img=$lb('#lbImg');
      if(!img||img.style.display==='none')return;
      const stR=lbStage.getBoundingClientRect(),imgR=img.getBoundingClientRect();
      const w=Math.round(imgR.width),h=Math.round(imgR.height);
      if(!w||!h)return;
      mkCanvas.width=w;mkCanvas.height=h;
      mkCanvas.style.width=w+'px';mkCanvas.style.height=h+'px';
      mkCanvas.style.left=Math.round(imgR.left-stR.left)+'px';
      mkCanvas.style.top=Math.round(imgR.top-stR.top)+'px';
      redrawMk();
    }

    function mkCanvasPos(e){
    ...
    // Drawing
    mkCanvas.addEventListener('mousedown',onMkDown);
    mkCanvas.addEventListener('mousemove',onMkMove);
    mkCanvas.addEventListener('mouseup',onMkUp);
    mkCanvas.addEventListener('mouseleave',onMkUp);
    ...
    function onMkDown(e){
      if(!mkActive)return;
      const pos=mkCanvasPos(e);
      if(mkTool==='text'){
        const stR=lbStage.getBoundingClientRect(),mkR=mkCanvas.getBoundingClientRect();
        const tw=$lb('#lbMkTextWrap');
        tw.style.left=Math.min(mkR.left-stR.left+pos.x,stR.width-200)+'px';
        tw.style.top=Math.max(mkR.top-stR.top+pos.y-44,4)+'px';
        tw.style.display='block';tw.dataset.x=pos.x;tw.dataset.y=pos.y;
        $lb('#lbMkTextVal').value='';
        setTimeout(()=>$lb('#lbMkTextVal').focus(),0);
        return;
      }
      mkDrawing=true;
      if(mkTool==='pen') mkCurrent={type:'pen',color:mkColor,width:mkWidth,points:[pos]};
      else               mkCurrent={type:mkTool,color:mkColor,width:mkWidth,x1:pos.x,y1:pos.y,x2:pos.x,y2:pos.y};
    }
    function onMkMove(e){
      if(!mkActive||!mkDrawing||!mkCurrent)return;
      const pos=mkCanvasPos(e);
      if(mkTool==='pen') mkCurrent.points.push(pos);
      else              {mkCurrent.x2=pos.x;mkCurrent.y2=pos.y;}
      redrawMk();
    }
    function onMkUp(){
      if(!mkDrawing||!mkCurrent)return;
      mkDrawing=false;
      const ok=mkTool==='pen'?mkCurrent.points.length>=2:Math.hypot(mkCurrent.x2-mkCurrent.x1,mkCurrent.y2-mkCurrent.y1)>4;
      if(ok){mkStrokes.push(mkCurrent);redrawMk();updateMkQueueBtn();}
      mkCurrent=null;
    }

    // Text commit
    $lb('#lbMkTextVal').addEventListener('keydown',e=>{if(e.key==='Enter')commitMkText();if(e.key==='Escape'){$lb('#lbMkTextWrap').style.display='none';mkDrawing=false;}e.stopPropagation();});
    $lb('#lbMkTextVal').addEventListener('blur',()=>setTimeout(commitMkText,150));
    function commitMkText(){
      const tw=$lb('#lbMkTextWrap');if(tw.style.display==='none')return;
      const txt=$lb('#lbMkTextVal').value.trim();
      if(txt){mkStrokes.push({type:'text',color:mkColor,width:mkWidth,x:parseFloat(tw.dataset.x),y:parseFloat(tw.dataset.y),text:txt});redrawMk();updateMkQueueBtn();}
      tw.style.display='none';
    }

    function redrawMk(){
      mkCtx.clearRect(0,0,mkCanvas.width,mkCanvas.height);
      mkStrokes.forEach(drawMkStroke);
      if(mkCurrent)drawMkStroke(mkCurrent);
    }
    function drawMkStroke(s){
      const ctx=mkCtx;
      ctx.save();ctx.strokeStyle=s.color;ctx.fillStyle=s.color;ctx.lineWidth=s.width;ctx.lineCap='round';ctx.lineJoin='round';
      switch(s.type){
        case 'pen':
          if(s.points.length<2){ctx.beginPath();ctx.arc(s.points[0].x,s.points[0].y,s.width/2,0,Math.PI*2);ctx.fill();}
          else{ctx.beginPath();ctx.moveTo(s.points[0].x,s.points[0].y);for(let i=1;i<s.points.length-1;i++){const mx=(s.points[i].x+s.points[i+1].x)/2,my=(s.points[i].y+s.points[i+1].y)/2;ctx.quadraticCurveTo(s.points[i].x,s.points[i].y,mx,my);}ctx.lineTo(s.points[s.points.length-1].x,s.points[s.points.length-1].y);ctx.stroke();}
          break;
        case 'circle':{const rx=Math.abs(s.x2-s.x1)/2,ry=Math.abs(s.y2-s.y1)/2,cx=(s.x1+s.x2)/2,cy=(s.y1+s.y2)/2;ctx.beginPath();ctx.ellipse(cx,cy,Math.max(rx,1),Math.max(ry,1),0,0,Math.PI*2);ctx.stroke();break;}
        case 'arrow':{const hl=Math.max(12,s.width*4),ang=Math.atan2(s.y2-s.y1,s.x2-s.x1);ctx.beginPath();ctx.moveTo(s.x1,s.y1);ctx.lineTo(s.x2,s.y2);ctx.stroke();ctx.beginPath();ctx.moveTo(s.x2,s.y2);ctx.lineTo(s.x2-hl*Math.cos(ang-Math.PI/6),s.y2-hl*Math.sin(ang-Math.PI/6));ctx.lineTo(s.x2-hl*Math.cos(ang+Math.PI/6),s.y2-hl*Math.sin(ang+Math.PI/6));ctx.closePath();ctx.fill();break;}
        case 'text':{const fs=Math.max(14,s.width*5);ctx.font=`bold ${fs}px sans-serif`;ctx.strokeStyle='rgba(0,0,0,.65)';ctx.lineWidth=s.width*2.5;ctx.strokeText(s.text,s.x,s.y);ctx.fillStyle=s.color;ctx.fillText(s.text,s.x,s.y);break;}
      }
      ctx.restore();
    }
    ...
    function setMkTool(t){
      mkTool=t;
      ['mkPen','mkCircle','mkArrow','mkText'].forEach(id=>$lb('#'+id).classList.toggle('sel',id==='mk'+t.charAt(0).toUpperCase()+t.slice(1)));
      mkCanvas.style.cursor=t==='text'?'text':'crosshair';
    }
    $lb('#mkPen').onclick=()=>setMkTool('pen');
    $lb('#mkCircle').onclick=()=>setMkTool('circle');
    $lb('#mkArrow').onclick=()=>setMkTool('arrow');
    $lb('#mkText').onclick=()=>setMkTool('text');
    $lb('#mkWidth').oninput=e=>{mkWidth=parseInt(e.target.value);};
    lb.querySelectorAll('.mk-dot').forEach(d=>{
      d.onclick=()=>{mkColor=d.dataset.c;lb.querySelectorAll('.mk-dot').forEach(x=>x.classList.remove('sel'));d.classList.add('sel');};
    });
    ...
    function updateMkQueueBtn(){
      ...
      btn.disabled=mkStrokes.length===0;
      if(mkStrokes.length&&pendingGigAttachment){btn.textContent='↺ Re-queue';btn.style.background='#0891b2';}
      else if(mkStrokes.length){btn.textContent='📎 Queue for GIG';btn.style.background='';}
    }
    ...
    $lb('#mkQueue').onclick=async()=>{
      ...
      try{
        const img=$lb('#lbImg');
        const w=img.naturalWidth||mkCanvas.width,h=img.naturalHeight||mkCanvas.height;
        const off=document.createElement('canvas');off.width=w;off.height=h;
        const ctx=off.getContext('2d');
        // Composite: original image + scaled markup
        ...
        pendingGigAttachment={blob,filename:`markup_${origName.replace(/\.[^.]+$/,'')}_${Date.now()}.png`};
        $lb('#lbMkQueuedBadge').style.display='block';
        btn.textContent='✓ Queued!';btn.style.background='#15803d';btn.disabled=false;
        // Show pill in feature card
        $('#mkPendingPill').style.display='inline-flex';
        setStatus('Markup queued — will attach to GIG on submit','ok');
      }catch(e){
        console.error('Markup capture:',e);
        btn.textContent='📎 Queue for GIG';btn.disabled=false;
        setStatus('Markup capture failed: '+e.message,'error');
      }
    };

    // Upload pending markup to a GIG feature after it's created/updated
    async function uploadPendingAttachment(objectId, layerUrl){
      if(!pendingGigAttachment)return;
      try{
        let token='';
        try{const esriId=await loadModule('esri/identity/IdentityManager');const cred=await esriId.getCredential(layerUrl).catch(()=>null);token=cred?.token||'';}catch(_){}
        const fd=new FormData();
        fd.append('attachment',pendingGigAttachment.blob,pendingGigAttachment.filename);
        fd.append('f','json');
        if(token)fd.append('token',token);
        const resp=await fetch(`${layerUrl}/${objectId}/addAttachment`,{method:'POST',body:fd});
        const json=await resp.json();
        const r=json.addAttachmentResult;
        if(!r?.success&&!r?.objectId)throw new Error(r?.error?.description||json.error?.message||'Upload failed');
        pendingGigAttachment=null;
        $('#mkPendingPill').style.display='none';
        setStatus('Markup attached to GIG ✓','ok');
      }catch(e){
        console.warn('Markup attachment upload failed:',e);
        setStatus('⚠ GIG created but markup upload failed: '+e.message,'error');
        // Don't rethrow — GIG was created successfully
      }
    }
    ...
    async function submitNewQc(){
      ...
      try{
        ...
        // Upload markup to the first new GIG if queued
        if(pendingGigAttachment&&okResults[0]?.objectId){
          await uploadPendingAttachment(okResults[0].objectId,gl.url);
        }
    ...
    async function submitClearReview(){
      ...
      try{
        ...
        // Upload markup to the existing GIG if queued
        if(pendingGigAttachment){await uploadPendingAttachment(oid,gl.url);}
    ...
    async function updateOriginatingFeature(globalId,newStatus){
      ...
      for(const lyr of layers.items){
        try{if(lyr===sketchLayer)continue;await lyr.load();if(!lyr.globalIdField)continue;
          ...
          const ur=up.updateFeatureResults?.[0];if(!(ur?.success===true||(ur?.success===undefined&&ur?.error===null)))throw new Error(ur?.error?.message||'Update failed');
          return;
        }catch(e){throw e;}
      }
    ...
    // ── Home & End Session ────────────────────────────────────────────
    function goHome(){
      if(currentPhase==='review'){const r=sessionLog.filter(e=>e.action==='qc_review').length;if(!confirm(r>0?`Return to filters? You've reviewed ${r} feature(s) — session data will be lost.`:'Return to filter screen?'))return;}
      stopTimer();if(highlightHandle){highlightHandle.remove();highlightHandle=null;}mapView.popup?.close();
      qcQueue=[];sessionLog=[];sessionStartTime=null;currentIndex=0;pendingGigAttachment=null;
      setPhase('query');$('#queryResults').style.display='none';$('#btnRefresh').disabled=true;
      $('#hdrSub').textContent='Configure filters to begin';setStatus('Ready — configure filters and query.','ok');
    }
    function endSessionEarly(){
      const r=sessionLog.filter(e=>e.action==='qc_review').length;
      if(!r){alert('No features reviewed yet — nothing to summarize.');return;}
      if(confirm(`End session now? You've reviewed ${r} of ${qcQueue.length} features.`))completeSession();
    }

    // ── Complete ──────────────────────────────────────────────────────
    function completeSession(){
      stopTimer();if(highlightHandle){highlightHandle.remove();highlightHandle=null;}
    ...
    function resetSession(clearForm=true){
      currentIndex=0;qcQueue=[];sessionLog=[];sessionStartTime=null;pendingGigAttachment=null;
      stopTimer();if(highlightHandle){highlightHandle.remove();highlightHandle=null;}mapView.popup?.close();
      if(clearForm){
        ['#dateFrom','#dateTo'].forEach(s=>$(s).value='');
        selectedWOs.clear();selectedPOs.clear();selectedJobs.clear();refreshMultiDisplays();
        $('#sortOrder').value='desc';$('#gigTypeFilter').value='';
        $('#queryResults').style.display='none';$('#btnRefresh').disabled=true;
        setSpatialMode('none');clearSketchGraphics();setDrawHint('','Select a tool above to start drawing');
      }
    ...
    function onKey(e){
      if(lb.classList.contains('open')){
        ...
        if(e.key==='m'||e.key==='M'){$lb('#lbMkToggle').click();return;}
        if(mkActive){
          if(e.key==='p'||e.key==='P'){setMkTool('pen');return;}
          if(e.key==='c'||e.key==='C'){setMkTool('circle');return;}
          if(e.key==='a'||e.key==='A'){setMkTool('arrow');return;}
          if(e.key==='t'||e.key==='T'){setMkTool('text');return;}
          if((e.ctrlKey||e.metaKey)&&e.key==='z'){$lb('#mkUndo').click();return;}
          return; // Don't pass other keys through when markup active
        }
    ...
    $('#btnQuery').onclick=runQuery; $('#btnRefresh').onclick=runQuery;
    $('#btnStart').onclick=startReview; $('#btnZoom').onclick=zoomToFeature;
    $('#btnSubmit').onclick=()=>mode==='new_qc'?submitNewQc():submitClearReview();
    $('#btnSkip').onclick=skipFeature; $('#btnPrev').onclick=prevFeature;
    $('#btnEndSession').onclick=endSessionEarly; $('#btnHome').onclick=goHome;
    $('#btnExport').onclick=exportReport; $('#btnStartOver').onclick=()=>resetSession(true);
    ...
    // ── Cleanup & register ────────────────────────────────────────────
    function cleanup(){
      stopTimer();document.removeEventListener('keydown',onKey);
      ...
      mapView.popup?.close();css.remove();box.remove();lb.remove();
    }
