  ...
  try {
      ...
      <div style="padding:14px;display:flex;flex-direction:column;gap:10px;">
        ...
        <div id="phaseReview" style="display:none;">
          ...
          <div class="card" style="margin-bottom:10px;">
            ...
            <!-- Skip photo transfer (clear review only) -->
    ...
    async function uploadPendingAttachment(objectId, layerUrl) {
      ...
      try {
        let token = '';
        try {
          const esriId = await loadModule('esri/identity/IdentityManager');
          const cred = await esriId.getCredential(layerUrl).catch(() => null);
          token = cred?.token || '';
        } catch (_) {}
        ...
        if (!resp.ok) throw new Error(`HTTP ${resp.status} ${resp.statusText} — verify the GIG layer URL and permissions`);

        const json = await resp.json();
        console.debug('[rqcw] addAttachment response:', json); // visible in DevTools for debugging

        // ArcGIS REST errors surface at the top level, not inside addAttachmentResult
        if (json.error) throw new Error(json.error.message || `Server error ${json.error.code ?? ''}`);

        const r = json.addAttachmentResult;
        if (!r) throw new Error('No addAttachmentResult in response — confirm that attachments are enabled on the GIG layer (22100) in the service definition');
        if (!r.success && !r.objectId) throw new Error(r.error?.description || r.error?.message || `Upload rejected by server (code: ${r.error?.code ?? 'unknown'})`);
    ...
    // Find originating feature using its GlobalID (stored in billing_area_code).
    // Returns { layer, layerUrl, objectId, globalId, supportsAttachments } or null.
    async function findOriginatingFeature(globalId) {
      ...
      for (const lyr of layers.items) {
        try {
          ...
          return { layer: lyr, layerUrl: lyr.url, objectId, globalId, supportsAttachments };
        } catch (_) {}
      }
      return null;
    }

    async function getLayerToken(url) {
      try { const esriId = await loadModule('esri/identity/IdentityManager'); const cred = await esriId.getCredential(url).catch(() => null); return cred?.token || ''; } catch (_) { return ''; }
    }

    async function restDeleteAttachments(layerUrl, objectId, ids, token) {
      if (!ids.length) return;
      const fd = new FormData();
      fd.append('attachmentIds', ids.join(','));
      fd.append('f', 'json');
      if (token) fd.append('token', token);
      const resp = await fetch(`${layerUrl}/${objectId}/deleteAttachments`, { method: 'POST', body: fd });
      if (!resp.ok) throw new Error(`HTTP ${resp.status} deleting attachments`);
      const json = await resp.json();
      if (json.error) throw new Error(json.error.message || 'Delete failed');
      const failed = (json.deleteAttachmentResults || []).filter(r => !r.success);
      if (failed.length) throw new Error(`${failed.length} attachment(s) failed to delete`);
    }

    // Download → delete → upload.  progressCb receives 0–100.
    // Throws on any failure; nothing is committed to the GIG/feature until doApproveGig runs.
    async function executePhotoTransfer(gigLayerUrl, gigOid, origInfo, toTransfer, toDeleteIds, progressCb) {
      ...
      // Step 1: Download GIG photos as blobs (validates data before anything is deleted)
      const blobs = [];
      for (const att of toTransfer) {
        const token = await getLayerToken(gigLayerUrl);
        const url = token ? `${att.url}${att.url.includes('?') ? '&' : '?'}token=${token}` : att.url;
        const resp = await fetch(url);
        if (!resp.ok) throw new Error(`Download failed for "${att.name}": HTTP ${resp.status}`);
        blobs.push({ blob: await resp.blob(), filename: att.name });
        tick();
      }

      // Step 2: Delete selected original feature photos
      if (toDeleteIds.length && origInfo?.supportsAttachments) {
        const token = await getLayerToken(origInfo.layerUrl);
        await restDeleteAttachments(origInfo.layerUrl, origInfo.objectId, toDeleteIds, token);
        tick();
      }

      // Step 3: Upload blobs to original feature
      if (origInfo?.supportsAttachments) {
        for (const { blob, filename } of blobs) {
          ...
          fd.append('f', 'json');
          if (token) fd.append('token', token);
          const resp = await fetch(`${origInfo.layerUrl}/${origInfo.objectId}/addAttachment`, { method: 'POST', body: fd });
          if (!resp.ok) throw new Error(`Upload failed for "${filename}": HTTP ${resp.status}`);
          const json = await resp.json();
          const r = json.addAttachmentResult;
    ...
    function openPhotoTransferPanel(item, notes, gigAtts, gigOid, gigLayerUrl, origInfo, origAtts) {
      ...
      // Build left column — GIG fix photos
      ...
      // Build right column — original feature photos
    ...
    // The actual GIG approval write — only called after photo transfer succeeds (or is bypassed).
    ...
    async function submitNewQc(){
      ...
      try{
        ...
        if(cfg.update){const oid=item.feature.attributes[item.layer.objectIdField];const up=await item.layer.applyEdits({updateFeatures:[{attributes:{[item.layer.objectIdField]:oid,workflow_status:cfg.wfStatus,workflow_stage:'OSP_CONST'}}]});const ur=up.updateFeatureResults?.[0];if(!(ur?.success===true||(ur?.success===undefined&&ur?.error===null)))throw new Error(ur?.error?.message||'Feature update failed');}
        if(pendingGigAttachment&&okResults[0]?.objectId)await uploadPendingAttachment(okResults[0].objectId,gl.url);
    ...
    async function submitClearReview() {
      ...
      // ── Re-open path (no photo transfer needed) ─────────────────────
      if (dv === 'reopen') {
        try {
          setStatus('Saving…','busy'); $('#btnSubmit').disabled = true;
          const gl = gigLayerRef(); await gl.load();
      ...
      // Fetch GIG attachments and check for transferable images
      try {
        ...
        // No transferable photos → bypass panel and approve directly
        if (!transferable.length) {
          $('#btnSubmit').disabled = false;
        ...
        // Find originating feature using its GlobalID
        const origInfo = await findOriginatingFeature(item.gisId);

        // If feature not found or layer doesn't support attachments → skip photo steps
        if (!origInfo) {
        ...
        // Fetch original feature's existing photos
        let origAtts = [];
        if (origInfo.supportsAttachments) {
          try {
        ...
        openPhotoTransferPanel(item, notes, gigAtts, gigOid, gl.url, origInfo, origAtts);
    ...
    async function updateOriginatingFeature(globalId, newStatus) {
      ...
      for (const lyr of layers.items) {
        try {
          ...
          const up = await lyr.applyEdits({ updateFeatures: [{ attributes: { [lyr.objectIdField]: oid, workflow_status: newStatus, workflow_stage: 'OSP_CONST' } }] });
